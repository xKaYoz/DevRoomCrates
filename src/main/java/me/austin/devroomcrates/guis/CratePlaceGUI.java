...
public class CratePlaceGUI implements DevRoomInventory {

    private final Location location;
    private int page;
    Chat chat = DevRoomCrates.getInstance().getChat();

    public CratePlaceGUI(Location location, int page) {
    ...
    public void onGUIClick(Inventory inv, Player p, int slot, ItemStack item, ClickType type) {
        ...
        if (slot >= 8 && slot <= 45) {
            ...
            if(type.isLeftClick()) {
                p.closeInventory(InventoryCloseEvent.Reason.PLUGIN);
                DevRoomCrates.getInstance().getCrateManager().createLocation(location, crate);
                chat.sendMessage(p, "&fYou have placed a '" + crate.getDisplayName() + "&f' crate at " +
                        "X&8:&e" + location.getBlockX() + "&f Y&8:&e" + location.getBlockY() + "&f Z&8:&e" + location.getBlockZ() + "&f.");
            }
        }
    }
    ...
    public @NotNull Inventory getInventory() {
        Inventory inv = Bukkit.createInventory(this, 54, chat.format("&bSelect a crate to place..."));
        ...
        for (t = start; t < end; t++) {
            ...
            lore.add("&a&oLeft click to select");
